...
function Start () {
	...
	if (usePoints) {
		var dotLine = new VectorLine("Spline", new List.<Vector3>(segments+1), 2.0, LineType.Points);
		dotLine.MakeSpline (splinePoints.ToArray(), segments, loop);
		dotLine.Draw();
	}
	else {
		var spline = new VectorLine("Spline", new List.<Vector3>(segments+1), 2.0, LineType.Continuous);
