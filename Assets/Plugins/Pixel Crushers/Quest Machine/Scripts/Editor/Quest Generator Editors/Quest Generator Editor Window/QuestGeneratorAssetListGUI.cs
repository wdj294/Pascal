    ...
    public class QuestGeneratorAssetListGUI
    {
        ...
        protected virtual void DrawAssetList(List<AssetInfo> assetInfoList, float width)
        {
            ...
            for (int i = 0; i < assetInfoList.Count; i++)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                if (GUILayout.Button(assetInfo.name, EditorStyles.textField, GUILayout.Width(fieldWidth))) Selection.activeObject = EditorUtility.InstanceIDToObject(assetInfo.instanceID);
